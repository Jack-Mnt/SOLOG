...
interface Intent { action: CashierAction; payload: Record<string, unknown>; content: string; recoveryUntil?: number }
...
export class CashierStore {
  ...
  private pendingCapabilitySession: string | null = null
  get needsCapabilityRefresh() { return this.pendingCapabilitySession !== null }
  get capability() {
    ...
    return this.pendingCapabilitySession ? { ...result, captureAllowed: false, deliveryAllowed: false } : result
  }
  ...
  dispose() {
    ...
    this.bootstrap = null
    this.intent = null
    this.pendingCapabilitySession = null
  ...
  async refresh(): Promise<void> {
    ...
    const request = this.transport.bootstrap(this.deviceToken).then((next) => {
      ...
      if (this.pendingCapabilitySession && !(this.capability.mode === 'expired' && !next.panel_state.session) && (next.panel_state.session?.id !== this.pendingCapabilitySession ||
        next.site.id !== previous?.site.id || next.device.id !== previous?.device.id)) {
  ...
  async startAndRefresh() {
    if (!this.pendingCapabilitySession) await this.mutate('start')
    await this.refresh()
  }
  async mutate(action: CashierAction, body: Record<string, unknown> = {}): Promise<CashierMutation> {
    ...
    if (this.intent?.recoveryUntil !== undefined &&
      (Date.now() + this.serverOffsetMs >= this.intent.recoveryUntil ||
        ...
        this.intent.payload.conteo_id !== b.panel_state.session?.id)) {
      throw new SologApiError('SOLOG_SESSION_EXPIRED')
    }
    // También se comprueba al reintentar: una intención incierta no autoriza entrega tardía ilimitada.
    ...
    const request = this.transport.mutate(intent.action, intent.payload).then((response) => {
      ...
      // Fijar el vencimiento antes de adoptar un timestamp de una respuesta demorada.
      ...
      if (response.revisions.operational >= (this.bootstrap?.revisions.operational ?? 0)) {
        ...
        if (action === 'start' && state) this.pendingCapabilitySession = state.session.id
      ...
      if (generation === this.generation && error instanceof SologApiError &&
        ...
        this.intent = null
        if (error.code === 'SOLOG_SESSION_EXPIRED' && b.panel_state.session) this.expiredSessions.add(b.panel_state.session.id)
