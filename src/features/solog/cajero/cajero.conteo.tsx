...
import type { CajeroGroupsResponse, CajeroStockType } from "./cajero.types";
import {
  deriveCajeroCategories,
  getCajeroCategoryIcon,
} from "./cajero.utils";
import { deriveCajeroProgress, initialCajeroStockType } from "./cajero.progress";
...
export function CajeroConteo({
  ...
  const [selectedType, setSelectedType] = useState<CajeroStockType | null>(
    () => initialCajeroStockType(window.location.search),
  );
  const [openCategoryId, setOpenCategoryId] = useState<string | null>(null);
  const [groupsState, setGroupsState] = useState<CajeroGroupsResponse | null>(
    null,
  );
  ...
  const progress = deriveCajeroProgress(bootstrap.panel_state, readCajeroBuffer(activeScope).items);
  const typeItems: CajeroSelectionGridItem[] = STOCK_TYPES.map((type) => ({
    id: type.id,
    name: type.name,
    icon: type.icon,
    count: progress.select(type.id).total,
    total: progress.select(type.id).total,
    completed: progress.select(type.id).completed,
  }));
  const effectiveType =
    selectedType
      ? selectedType
      : ((typeItems.find((item) => item.count > 0)?.id as
          | CajeroStockType
          | undefined) ?? null);
  const typeGroupIds = effectiveType ? progress.select(effectiveType).ids : new Set<string>();
  const categories = effectiveType
    ? deriveCajeroCategories(groups.filter((group) => typeGroupIds.has(group.grupo_id)))
    : [];
  const categoryItems: CajeroSelectionGridItem[] = categories.map(
    (category) => ({
      id: category.id,
      name: category.nombre,
      count: category.count,
      total: progress.select(effectiveType!, category.id).total,
      completed: progress.select(effectiveType!, category.id).completed,
  ...
  const modalGroups =
    openCategory && effectiveType
      ? groups.filter((group) => typeGroupIds.has(group.grupo_id) && group.categoria_id === openCategory.id)
      : [];
  ...
  const nextCategory =
    openCategoryIndex >= 0
      ? categories
          .slice(openCategoryIndex + 1)
          .find((category) => category.count > 0)
      : undefined;
  const modalView = STOCK_TYPES.find((type) => type.id === effectiveType)?.view;

  return (
    <section
      className="cajero-module cajero-operational"
      aria-labelledby="cajero-conteo-title"
    >
      ...
      {groupsState ? (
        <>
          ...
          {effectiveType ? (
            <section
              className="cajero-selection-level"
              aria-labelledby="cajero-stock-category-title"
            >
      ...
      {openCategory && modalView && modalGroups.length > 0 ? (
        <CajeroCaptureModal
          ...
          onNextCategory={
            nextCategory ? () => setOpenCategoryId(nextCategory.id) : undefined
          }
