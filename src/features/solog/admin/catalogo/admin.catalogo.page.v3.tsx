...
import { Check, CircleOff, ClipboardCheck, DollarSign, Eye, EyeOff, Package, RefreshCw, RotateCcw, Settings, Undo2, Upload, Wrench } from 'lucide-react'
...
<div
    ...
    >
      {proposalStatuses.map((item) => <button type="button" role="tab" id={`admin-catalog-state-${item.id}`} key={item.id} aria-selected={status === item.id} aria-controls="admin-catalog-proposals-panel" tabIndex={status === item.id ? 0 : -1} onClick={() => setStatus(item.id)}>{item.label}{query.data && <strong>{query.data.counts[item.id]}</strong>}</button>)}
    </div>
  ...
  return <>
    <AdminDialog title={proposal.producto ?? `Propuesta ${proposal.c_interno}`} description={proposalLabels[proposal.tipo]} onClose={onClose} closeDisabled={!!intent?.pending} wide>
      ...
      <div className="admin-v2-actions">
        {proposal.estado === 'pendiente' && <><button type="button" className="button" disabled={!!intent} onClick={() => run('approve')}><Check size={16} aria-hidden="true" />Aprobar</button><button type="button" className="button button--secondary" disabled={!!intent} onClick={() => run('ignore')}><EyeOff size={16} aria-hidden="true" />Ignorar propuesta</button></>}
        {proposal.estado === 'aprobado' && <><button type="button" className="button button--secondary" disabled={!!intent} onClick={() => run('withdraw')}><Undo2 size={16} aria-hidden="true" />Retirar aprobación</button>{canSetup && <button type="button" className="button" disabled={!!intent} onClick={() => setSetup(true)}><Settings size={16} aria-hidden="true" />{proposal.setup ? 'Actualizar configuración' : 'Configurar producto'}</button>}{proposal.tipo === 'precio' && <button type="button" className="button" disabled={!!intent} onClick={() => setPrice(true)}><DollarSign size={16} aria-hidden="true" />{proposal.price_resolution ? 'Actualizar resolución de precio' : 'Resolver precio'}</button>}</>}
      </div>
...
function PriceResolutionDialog({ fingerprint, onClose, onComplete }: { fingerprint: string; onClose: () => void; onComplete: () => void }) {
  ...
  return <><AdminDialog title="Resolver precio" description={`C. interno ${options.c_interno}`} onClose={onClose} closeDisabled={!!intent?.pending} wide>
    ...
    {resolution && <fieldset><legend>Valorizado</legend>{canKeep && <button type="button" className="button button--secondary" aria-pressed={packageAction === 'keep'} onClick={() => setPackageAction('keep')}><Package size={16} aria-hidden="true" />Conservar valorizado</button>}<button type="button" className="button button--secondary" aria-pressed={packageAction === 'set' || packageAction === 'clear'} onClick={() => setValuation(true)}><RefreshCw size={16} aria-hidden="true" />Actualizar valorizado</button>{resolution === 'separate_sku' && <button type="button" className="button button--secondary" aria-pressed={packageAction === 'not_applicable'} onClick={() => setPackageAction('not_applicable')}><CircleOff size={16} aria-hidden="true" />Sin valorizado</button>}{packageAction === 'clear' && <p>El valorizado se eliminará al publicar.</p>}{packageAction === 'set' && preparedValuation && <p>Se preparará x{preparedValuation.unidades_por_paquete} · S/ {preparedValuation.precio_paquete.toFixed(2)} al publicar.</p>}</fieldset>}
    {intent && <CatalogIntentNotice onRetry={retry} />}
    <button type="button" className="button" disabled={!!intent || options.change_state !== 'aprobado'} onClick={submit}><Wrench size={16} aria-hidden="true" />Preparar resolución</button>
    {error && <p role="alert">{error}</p>}
...
function PublicationDialog({ onClose }: { onClose: () => void }) {
  ...
  return <AdminDialog title="Publicar catálogo" onClose={onClose} closeDisabled={!!receipt.pending} wide>
    ...
    <button type="button" className="button" disabled={!admin || !!receipt.pending || (!receipt.operationId && !preview?.ok)} onClick={publish}>{receipt.pending ? <RefreshCw size={16} aria-hidden="true" /> : receipt.operationId ? <RotateCcw size={16} aria-hidden="true" /> : <Upload size={16} aria-hidden="true" />}{receipt.pending ? 'Publicando…' : receipt.operationId ? 'Recuperar publicación' : 'Confirmar publicación'}</button>
...
export function AdminCatalogV3() {
  ...
  const store = useCatalogStore()
  return <section className="admin-catalog"><header className="admin-catalog__header"><div><CatalogStatus /></div><button type="button" className="button" onClick={() => setPublishing(true)}>{store.publication.operationId ? <RotateCcw size={16} aria-hidden="true" /> : <ClipboardCheck size={16} aria-hidden="true" />}{store.publication.operationId ? 'Recuperar publicación' : 'Revisar publicación'}</button></header><ProposalsSurface />{publishing && <PublicationDialog onClose={() => setPublishing(false)} />}</section>
}