...
export interface CatalogEnvelope { contract_version: 4; generated_at: string; revisions: CatalogRevisions }
...
type CatalogPriceResolution =
  | { resolution: 'update_group_price' | 'keep_structure'; package_action: 'keep' | 'clear' }
  | { resolution: 'update_group_price' | 'keep_structure'; package_action: 'update'; precio_paquete: number }
  | { resolution: 'update_group_price' | 'keep_structure'; package_action: 'set'; unidades_por_paquete: number; precio_paquete: number }
  | { resolution: 'separate_sku'; package_action: 'clear' | 'not_applicable' }
  | { resolution: 'separate_sku'; package_action: 'set'; unidades_por_paquete: number; precio_paquete: number }

export interface CatalogMutations {
  proposal_action: CatalogMutationBase & { propuesta_fingerprint: string; action: 'approve' | 'ignore' | 'reactivate' | 'withdraw' | 'discard' }
  ...
  resolve_product: CatalogMutationBase & ({ propuesta_fingerprint: string } & CatalogProductSetup)
  resolve_price: CatalogMutationBase & ({ propuesta_fingerprint: string } & CatalogPriceResolution)
  prepare_product: CatalogMutationBase & ({ propuesta_fingerprint: string } & CatalogProductSetup)
  prepare_price: CatalogMutationBase & ({ propuesta_fingerprint: string } & CatalogPriceResolution)
}
...
function envelope(value: unknown): asserts value is CatalogEnvelope & CatalogPayload {
  assert(object(value) && value.contract_version === 4 && timestamp(value.generated_at) && object(value.revisions) && integer(value.revisions.catalog) && integer(value.revisions.groups), 'Envelope Catálogo V4 inválido.')
}
function group(value: unknown, needsCategory = true) {
  assert(object(value) && typeof value.id === 'string' && typeof value.nombre === 'string' && number(value.precio) && (value.unidades_por_paquete === null || integer(value.unidades_por_paquete)) && (value.precio_paquete === null || number(value.precio_paquete)) && (!needsCategory || typeof value.categoria_id === 'string' && typeof value.categoria === 'string'), 'Grupo Catálogo V4 inválido.')
}
function complete(rows: unknown, total: unknown, isComplete: unknown, label: string): asserts rows is unknown[] {
  assert(Array.isArray(rows) && rows.length <= 10_000 && integer(total) && isComplete === true && total === rows.length, `${label} Catálogo V4 incompleto o truncado.`)
}
function proposal(value: unknown) {
  assert(object(value) && typeof value.propuesta_fingerprint === 'string' && /^[a-f0-9]{64}$/i.test(value.propuesta_fingerprint) && nullableString(value.cambio_id) && integer(value.c_interno) && inSet(value.tipo, types) && inSet(value.estado, statuses) && inSet(value.origen, origins) && inSet(value.seccion, ['urgente', 'emergente']) && object(value.datos) && nullableString(value.producto) && Array.isArray(value.sedes) && value.sedes.every(site => object(site) && typeof site.id === 'string' && typeof site.nombre === 'string') && integer(value.occurrence_count) && timestamp(value.first_seen_at) && timestamp(value.last_seen_at) && object(value.catalogo_actual) && nullableString(value.catalogo_actual.producto) && nullableString(value.catalogo_actual.c_barras) && (value.catalogo_actual.precio === null || number(value.catalogo_actual.precio)) && nullableString(value.catalogo_actual.marca) && (value.catalogo_actual.estado === null || inSet(value.catalogo_actual.estado, modes)) && nullableString(value.catalogo_actual.categoria) && nullableString(value.catalogo_actual.grupo) && typeof value.stale === 'boolean' && (value.publicable === null || typeof value.publicable === 'boolean') && nullableString(value.block_reason) && (value.setup === null || object(value.setup)) && (value.price_resolution === null || object(value.price_resolution)) && nullableString(value.aprobado_at) && nullableString(value.ignorado_at) && (value.version_aplicada === null || integer(value.version_aplicada)) && nullableString(value.incorporado_at), 'Propuesta Catálogo V4 inválida.')
...
export function validateCatalogRead<A extends CatalogReadAction>(action: A, value: unknown): CatalogReads[A] {
  ...
  if (action === 'status') {
    ...
    assert(object(catalog) && (catalog.version_actual === null || integer(catalog.version_actual)) && (catalog.publicado_at === null || timestamp(catalog.publicado_at)) && integer(catalog.incluidos) && integer(catalog.excluidos) && integer(catalog.total) && catalog.total === catalog.incluidos + catalog.excluidos, 'Status Catálogo V4 inválido.')
  }
  if (action === 'reference') {
    assert(Array.isArray(response.categories) && Array.isArray(response.groups), 'Reference Catálogo V4 inválido.')
    response.categories.forEach(category => assert(object(category) && typeof category.id === 'string' && typeof category.nombre === 'string' && integer(category.orden), 'Categoría Catálogo V4 inválida.'))
    response.groups.forEach(item => group(item))
  }
  if (action === 'proposals') {
    complete(response.rows, response.total, response.complete, 'Propuestas')
    const counts = response.counts
    assert(inSet(response.estado, statuses) && object(counts) && statuses.every(status => integer(counts[status])), 'Resumen de propuestas Catálogo V4 inválido.')
    response.rows.forEach(proposal)
  }
  if (action === 'products') {
    complete(response.rows, response.total, response.complete, 'Productos')
    assert(Array.isArray(response.setup_required), 'setup_required Catálogo V4 inválido.')
    response.rows.forEach(product)
    response.setup_required.forEach(item => assert(object(item) && typeof item.cambio_id === 'string' && typeof item.propuesta_fingerprint === 'string' && inSet(item.tipo, ['agregar_producto', 'reincorporar_producto']) && integer(item.c_interno) && typeof item.producto === 'string' && number(item.precio) && item.setup === null && item.block_reason === 'configuracion_requerida', 'Elemento setup_required Catálogo V4 inválido.'))
  }
  if (action === 'price_options') {
    assert(typeof response.propuesta_fingerprint === 'string' && nullableString(response.change_id) && inSet(response.change_state, statuses) && integer(response.c_interno) && number(response.nuevo_precio) && Array.isArray(response.members) && Array.isArray(response.options) && response.options.every(option => inSet(option, ['update_group_price', 'separate_sku', 'keep_structure'])) && typeof response.package_decision_required === 'boolean' && (response.prepared_resolution === null || object(response.prepared_resolution)) && Array.isArray(response.equivalent_proposals) && Array.isArray(response.conflicting_proposals), 'Opciones de precio Catálogo V4 inválidas.')
    group(response.grupo, false)
    response.members.forEach(member => assert(object(member) && integer(member.c_interno) && typeof member.producto === 'string' && number(member.precio), 'Miembro de grupo Catálogo V4 inválido.'))
    ;[...response.equivalent_proposals, ...response.conflicting_proposals].forEach(item => assert(object(item) && typeof item.propuesta_fingerprint === 'string' && integer(item.c_interno) && typeof item.producto === 'string' && number(item.nuevo_precio), 'Propuesta relacionada de precio Catálogo V4 inválida.'))
  }
  if (action === 'publication_preview') {
    assert(object(response.preview), 'Preview Catálogo V4 inválido.')
    const preview = response.preview
    assert(typeof preview.ok === 'boolean' && typeof preview.codigo === 'string' && Array.isArray(preview.conflictos) && preview.conflictos.every(object) && Array.isArray(preview.errores) && preview.errores.every(error => typeof error === 'string'), 'Preview Catálogo V4 inválido.')
    if (preview.ok) {
      const changes = preview.cambios
      assert((preview.version_actual === null || integer(preview.version_actual)) && (preview.version_nueva === null || integer(preview.version_nueva)) && integer(preview.schema_version) && integer(preview.sku_actuales) && integer(preview.sku_resultantes) && integer(preview.cambios_total) && object(changes) && types.every(type => integer(changes[type])) && Array.isArray(preview.change_ids) && preview.change_ids.every(id => typeof id === 'string'), 'Preview Catálogo V4 inválido.')
    } else {
...
export function validateCatalogMutationPayload(action: CatalogMutationAction, value: unknown) {
  assert(object(value) && typeof value.operation_id === 'string' && /^[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i.test(value.operation_id) && integer(value.expected_catalog_revision) && integer(value.expected_groups_revision), 'Payload base Catálogo V4 inválido.')
  if (action === 'proposal_action') {
    assert(typeof value.propuesta_fingerprint === 'string' && inSet(value.action, ['approve', 'ignore', 'reactivate', 'withdraw', 'discard']), 'Payload proposal_action inválido.')
    return
  }
  const validateSetup = () => {
    assert(inSet(value.mode, ['existing_group', 'new_unit']) && (value.marca === undefined || nullableString(value.marca)) && (value.mode === 'existing_group' ? typeof value.grupo_id === 'string' : typeof value.categoria_id === 'string'), 'Configuración de producto inválida.')
  }
  const validatePrice = () => {
    assert(inSet(value.resolution, ['update_group_price', 'separate_sku', 'keep_structure']), 'Resolución de precio inválida.')
    const resolution = value.resolution
    const packageAction = value.package_action
    const set = packageAction === 'set' && integer(value.unidades_por_paquete) && value.unidades_por_paquete > 1 && number(value.precio_paquete) && value.precio_paquete > 0
    const update = packageAction === 'update' && number(value.precio_paquete) && value.precio_paquete > 0
    if (resolution === 'separate_sku') assert(packageAction === 'clear' || packageAction === 'not_applicable' || set, 'Decisión de valorizado para separación inválida.')
    else assert(packageAction === 'keep' || packageAction === 'clear' || update || set, 'Decisión de valorizado inválida.')
  }

  if (action === 'propose_product_state') {
    assert(integer(value.c_interno) && inSet(value.action, ['exclude', 'reincorporate']), 'Payload propose_product_state inválido.')
    if (value.action === 'reincorporate') validateSetup()
    return
  }
  if (action === 'resolve_product' || action === 'prepare_product') {
    assert(typeof value.propuesta_fingerprint === 'string', `Payload ${action} inválido.`)
    validateSetup()
    return
  }
...
export function validateCatalogMutation(value: unknown): CatalogMutationResult {
  envelope(value)
  assert(typeof value.replay === 'boolean' && object(value.result), 'Mutación Catálogo V4 inválida.')
...
export async function catalogRead<A extends CatalogReadAction>(action: A, payload: CatalogReadPayloads[A]): Promise<CatalogReads[A]> {
  ...
  const { data, error } = await supabase.rpc('rpc_solog_admin_catalog_read_v4', { p_action: action, p_payload: payload })
...
export async function catalogMutate<A extends CatalogMutationAction>(action: A, payload: CatalogMutations[A]): Promise<CatalogMutationResult> {
  ...
  if (!supabase) throw createSologConfigurationError()
  const { data, error } = await supabase.rpc('rpc_solog_admin_catalog_v4', { p_action: action, p_payload: payload })
  if (error) throw normalizeSologError(error)
...
export function validateCatalogPublication(value: unknown, operationId: string): CatalogPublicationResult {
  ...
  if (typeof value.replay !== 'boolean' || typeof value.completion_recorded !== 'boolean' || !integer(value.version) || typeof value.hash !== 'string' || typeof value.storage_path !== 'string' || !integer(value.productos) || !integer(value.grupos_activos) || !integer(value.cambios_incorporados)) throw new CatalogPublicationError('Respuesta de publicación Catálogo V4 incompleta.', true)
