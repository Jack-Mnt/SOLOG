...
export function AdminOverview({
  ...
  const coverage = dashboard.kpis.cobertura_periodo
    ...
    <div className="admin-dashboard">
      <section className="admin-dashboard-kpis" aria-label="Indicadores principales">
        <article className="admin-dashboard-kpi admin-dashboard-kpi--coverage">
          ...
          <div><span>Cobertura del período</span><strong>{coverage.porcentaje}%</strong><small>{coverage.grupos_contados} / {coverage.grupos_totales} grupos</small></div>
          <progress aria-label="Cobertura del período global" max="100" value={coverage.porcentaje}>{coverage.porcentaje}%</progress>
        </article>
      ...
      <section className="admin-dashboard-sites" aria-labelledby="sites-title">
        <div className="admin-dashboard-sites__toolbar">
          <div><h2 id="sites-title">Resumen por sede</h2><p>Período: {formatPeriodDate(dashboard.periodo.periodo_desde)} – {formatPeriodDate(dashboard.periodo.periodo_hasta)}.</p></div>
        </div>
        ...
        {dashboard.sedes.length > 0 ? (
          <div className="admin-dashboard-table-wrap">
            <table className="admin-dashboard-table">
              ...
              <thead><tr><th>Sede</th><th>Cobertura del período</th><th>Cobertura diaria</th><th>Por recontar</th><th>Confirmadas</th><th>Inconsistentes</th><th>Actividad</th><th aria-label="Abrir actividad" /></tr></thead>
              <tbody>
                {dashboard.sedes.map((site) => (
                  ...
                  >
                    ...
                    <td><CoverageCell coverage={site.cobertura_periodo} label={`Cobertura del período de ${site.sede}`} /></td>
