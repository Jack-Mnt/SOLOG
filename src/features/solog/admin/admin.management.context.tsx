...
export function useManagementQuery<A extends ReadAction>(action: A, payload: ReadPayloads[A], options: { enabled?: boolean } = {}) {
  ...
  useEffect(() => {
    if (!enabled || expiry === undefined) return
  ...
  }, [action, key, store, expiry, enabled])
  useEffect(() => {
    let active = true
    queueMicrotask(() => { if (enabled && active && !store.peek(action, JSON.parse(key)).data && !store.peek(action, JSON.parse(key)).error) void store.load(action, JSON.parse(key)).catch(() => {}) })
    return () => { active = false }
  }, [action, key, store, version, enabled])
