...
export function QueryState({ error, retry }: { error?: string; retry: () => void }) {
  return <div className={error ? 'notice notice--error' : 'notice'} role={error ? 'alert' : 'status'}>{error ?? 'Cargando…'}{error && <button className="button" onClick={retry}><RotateCcw size={16} aria-hidden="true" />Reintentar</button>}</div>
}
