...
export class AdminStore {
  ...
  private observe(response: Envelope, site?: string) {
    if (response.revisions.groups !== undefined) {
      ...
      if (response.revisions.groups > this.groups) { this.groups = response.revisions.groups; this.invalidate() }
    }
  ...
  current = () => this.live
  dispose() { this.live = false; this.epoch++; this.entries.clear(); this.bootstrap = null; this.operational.clear(); this.listeners.clear() }
  ...
  async load<A extends AdminAction>(action: A, payload: AdminPayloads[A]): Promise<AdminResponses[A]> {
    ...
    const request = this.rpc(action, payload).then(response => {
      ...
      if (action === 'bootstrap') {
        ...
        if (this.bootstrap && (b.identity.rol !== this.bootstrap.identity.rol || JSON.stringify(b.allowed_sites.map(s => s.id)) !== JSON.stringify(this.bootstrap.allowed_sites.map(s => s.id)))) this.invalidate()
        this.bootstrap = b
