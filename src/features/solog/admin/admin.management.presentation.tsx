...
export function ReadNotice({ error, retry }: { error?: string; retry: () => void }) { return <p role={error ? 'alert' : 'status'}>{error ?? 'Cargando…'}{error && <button type="button" className="button" onClick={retry}><RotateCcw size={16} aria-hidden="true" />Reintentar lectura</button>}</p> }
