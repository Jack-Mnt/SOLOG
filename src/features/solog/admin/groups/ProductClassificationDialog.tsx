...
export function ProductClassificationDialog({ initialProduct, initialGroup, reference, saving, onClose, onSave }: {
  ...
  return (
    <AdminDialog closeDisabled={saving} footer={<><button className="button button--secondary" disabled={saving} onClick={() => initialProduct ? onClose() : setSelected(null)} type="button"><ArrowLeft size={17} />{initialProduct ? 'Cancelar' : 'Volver'}</button><button className="button" disabled={saving || (mode === 'Agrupado' && !groupId)} onClick={() => void submit()} type="button">{saving ? <LoaderCircle className="icon-spin" size={17} /> : <Save size={17} />}Guardar</button></>} onClose={onClose} title="Cambiar clasificación">
      ...
      <div className="group-change-form"><label>Modalidad<select disabled={saving} onChange={(event) => { const next = event.target.value as typeof mode; setMode(next); if (next !== 'Agrupado') setGroupId('') }} value={mode}><option value="Único">Único — conteo individual</option><option value="Agrupado">Agrupado — grupo compatible</option><option value="Excluido">Excluido — fuera del conteo</option></select></label>{mode === 'Agrupado' ? <label>Grupo destino<select disabled={saving} onChange={(event) => setGroupId(event.target.value)} required value={groupId}><option value="">Seleccionar…</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.nombre} · {formatAdminCurrency(group.precio)}</option>)}</select></label> : <p className="helper-text">{mode === 'Único' ? 'El producto se contará individualmente.' : 'El producto quedará fuera del conteo.'}</p>}{error ? <div className="notice notice--error" role="alert">{error}</div> : null}</div>
    </AdminDialog>
  )
}
