...
export function GroupsPanel({ refreshOperationalState }: { refreshOperationalState: () => Promise<void> }) {
  ...
  return (
    <section className="content-section admin-module groups-workbench" aria-labelledby="groups-title">
      <div className="section-heading"><div><div className="section-title-row"><span className="section-icon"><Layers3 size={20} /></span><h2 id="groups-title">Grupos</h2></div><p>Estructura y valorización de grupos de conteo.</p></div><button className="button" disabled={!groups.reference} onClick={() => setDefinition('new')} type="button"><Plus size={17} />Nueva definición</button></div>
