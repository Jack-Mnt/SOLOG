...
import { ArrowLeft, ArrowRight, ChevronRight, Pencil, Plus, Search, Shuffle } from 'lucide-react'
...
export function GroupsPanel({ refreshOperationalState }: { refreshOperationalState: () => Promise<void> }) {
  ...
  return (
    <div className="content-section admin-module groups-workbench">
      <div className="groups-workbench__toolbar"><div className="groups-legend"><span><i className="groups-legend__published" />Publicado actual</span><span><i className="groups-legend__proposed" />Cambio futuro</span></div><button className="button" disabled={!groups.reference} onClick={() => setDefinition('new')} type="button"><Plus size={17} />Nueva definición</button></div>
      <form className="groups-filters admin-filter-bar" onSubmit={submitFilters}><label className="admin-filter-field admin-filter-search-field">Buscar<span className="catalog-search__control admin-filter-search-control"><Search size={16} /><input disabled={groups.status === 'loading'} onChange={(event) => groups.setDraftFilters((current) => ({ ...current, buscar: event.target.value }))} placeholder="Grupo, producto o C. interno…" value={groups.draftFilters.buscar} /></span></label><label className="admin-filter-field admin-filter-field--select">Categoría<select disabled={groups.status === 'loading'} onChange={(event) => groups.setDraftFilters((current) => ({ ...current, categoria_id: event.target.value }))} value={groups.draftFilters.categoria_id}><option value="">Todas</option>{groups.reference?.categorias.map((category) => <option key={category.id} value={category.id}>{category.nombre}</option>)}</select></label><label className="admin-filter-field admin-filter-field--select">Tipo<select disabled={groups.status === 'loading'} onChange={(event) => groups.setDraftFilters((current) => ({ ...current, tipo: event.target.value as '' | 'Único' | 'Agrupado' }))} value={groups.draftFilters.tipo}><option value="">Todos</option><option value="Único">Único</option><option value="Agrupado">Agrupado</option></select></label><div className="admin-report-filter-actions admin-filter-actions"><button className="button admin-filter-apply" disabled={groups.status === 'loading'} type="submit">Aplicar</button><button className="button button--secondary" disabled={groups.status === 'loading'} onClick={groups.resetFilters} type="button">Limpiar</button></div></form>
