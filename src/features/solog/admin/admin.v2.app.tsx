...
function Shell({ route, onLogout, legacy }: { route: AdminRoute; onLogout: () => void; legacy: ReactNode }) {
  const store = useAdminStore()
  const bootstrap = useAdminQuery('bootstrap', {})
  const [collapsed, setCollapsed] = useState(false)
  const deferred = route !== '/admin' && route !== '/admin/control'
  useEffect(() => {
    // Deferred modules still own legacy mutations; discard operational caches on return.
    if (deferred) return () => store.refresh()
  }, [deferred, store])
  ...
  return <main className={`admin-workspace admin-v2-workspace${collapsed ? ' admin-workspace--collapsed' : ''}`}>
    ...
    <section className="admin-workspace__main"><header className="admin-header"><h1>{route === '/admin' ? 'Dashboard' : 'Control'}</h1><button className="button button--secondary" onClick={() => store.refresh()}>Actualizar Admin</button></header>
      <div className="admin-workspace__content"><Suspense fallback={<PanelLoader contained />}>{route === '/admin' ? <Dashboard /> : <Control />}</Suspense></div>
    </section>
  </main>
}
export function AdminV2App(props: { userId: string; route: AdminRoute; onLogout: () => void; legacy: ReactNode }) {
