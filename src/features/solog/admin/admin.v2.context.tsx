...
export function useAdminQuery<A extends AdminAction>(action: A, payload: AdminPayloads[A], options: { enabled?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (enabled && active && !cached.data && !cached.error) void store.load(action, p).catch(() => {})
    })
    return () => { active = false }
  }, [action, enabled, key, store, version])
