...
export interface ReadPayloads {
  summary: { site_id?: string }; detail_sites: { family_key: string }; list: { site_id?: string }
}
export interface Mutations {
  ...
  authorize: { device_id: string }; revoke: { device_id: string }; reject: { device_id: string }
}
...
export function domain(action: ReadAction | MutationAction): Domain {
  if (['summary', 'detail_sites', 'ignore_30d', 'reactivate', 'propose_delete'].includes(action)) return 'incidents'
  return 'devices'
}
...
export function validateRead<A extends ReadAction>(action: A, value: unknown): Reads[A] {
  ...
  const rows = action === 'summary' ? value.families : action === 'detail_sites' ? value.sites : value.devices
