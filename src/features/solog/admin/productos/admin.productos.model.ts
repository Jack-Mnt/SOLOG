...
export function filterAndSortProducts(rows: MasterDataProduct[], derived: MasterDataDerived, filters: { search: string; mode: ProductModeFilter; categoryId: string; sort: ProductSort }) {
  ...
  return rows.filter((product) => {
    ...
    return (!term || searchable.includes(term)) && (filters.mode === 'all' || product.estado === filters.mode) && (filters.categoryId === 'all' || product.categoria_id === filters.categoryId)
