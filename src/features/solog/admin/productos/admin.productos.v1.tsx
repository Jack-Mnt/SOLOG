...
export function AdminProductsV1() {
  ...
  return <section className="admin-catalog admin-products">
    {masterData.snapshot.setup_required.length > 0 && <section className="admin-catalog__section admin-catalog__section--urgent"><header><h3>Configuración pendiente</h3><span>{masterData.snapshot.setup_required.length}</span></header><div className="admin-catalog__proposal-context">{masterData.snapshot.setup_required.map((item) => <span key={item.propuesta_fingerprint}>{item.c_interno} · {item.producto} · configuración requerida antes de publicar <button type="button" className="button button--secondary" onClick={() => setSetup(item)}>Configurar</button></span>)}</div></section>}
    <section className="admin-catalog__section"><header><div><h2>Productos</h2><p>{products.length} de {masterData.snapshot.totals.products} productos cargados completos.</p></div></header>
      ...
      <div className="admin-v2-table admin-catalog__table"><table><thead><tr><th scope="col">Producto</th><th scope="col">C. interno</th><th scope="col">Categoría</th><th scope="col">Grupo</th><th scope="col">Precio</th><th scope="col">Estado</th><th scope="col">Acciones</th></tr></thead><tbody>{visible.rows.map((product) => {
        ...
        return <tr key={product.c_interno}><th scope="row">{product.producto}</th><td>{product.c_interno}</td><td>{category}</td><td>{group ?? '—'}</td><td className="admin-catalog__money"><Value value={product.precio} money /></td><td>{product.estado === 'Excluido' ? 'Excluido' : 'Incluido'} · {product.estado}{product.propuesta ? ` · propuesta ${product.propuesta.estado}` : ''}</td><td><button type="button" className="button button--secondary" onClick={() => setSelected(product)}>{actionLabel}</button></td></tr>
