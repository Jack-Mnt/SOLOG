...
export function AdminCategoriesDialog({ onClose }: { onClose: () => void }) {
  ...
  return <AdminDialog title="Administrar categorías" description="Crea, renombra y define el orden operativo de las categorías." onClose={onClose} closeDisabled={!!intent?.pending} variant="wide" footer={<><button type="button" className="button button--secondary" disabled={!!intent?.pending} onClick={onClose}>Cerrar</button><button type="button" className="button" disabled={!!intent || !hasCurrentOrderDraft || order.length !== masterData.snapshot?.categories.length} onClick={() => void saveOrder()}><Save size={16} aria-hidden="true" />Guardar orden completo</button></>}>
    {!masterData.snapshot || !masterData.derived ? <QueryState error={masterData.error} retry={masterData.retry} variant="compact" /> : <>
