...
function GroupEditor({ group, product, onClose }: { group?: MasterGroup | null; product?: MasterProduct; onClose: () => void }) {
  ...
  const [mode, setMode] = useState<'Único' | 'Agrupado'>(product?.estado === 'Agrupado' ? 'Agrupado' : 'Único'), [destination, setDestination] = useState(product?.grupo_id ?? ''), [members, setMembers] = useState<number[]>([]), [error, setError] = useState('')
