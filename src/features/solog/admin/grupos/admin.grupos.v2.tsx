...
function Valuation({ group }: { group: Pick<MasterDataGroup, 'unidades_por_paquete' | 'precio_paquete'> }) {
  return group.unidades_por_paquete !== null && group.precio_paquete !== null ? <>x{group.unidades_por_paquete} · <Value value={group.precio_paquete} money /></> : null
}
...
export function AdminGroupsV2() {
  ...
  return <section className="admin-groups">
    ...
    <div className="admin-v2-table admin-groups__table" role="region" aria-label="Lista de grupos" tabIndex={0}><table><thead><tr><th scope="col">Grupo</th><th scope="col">Categoría</th><th scope="col">Integrantes</th><th scope="col">Valorizado</th><th scope="col">Editar</th></tr></thead><tbody>{visible.map(group => <tr key={group.id}><th scope="row">{group.nombre}</th><td>{group.categoryName}</td><td><button type="button" className="button button--secondary" onClick={() => setMembers(group.id)}><Users size={16} aria-hidden="true" />{group.derivedType === 'Único' ? 'Único' : `${group.memberCount} SKU`}</button></td><td className="admin-groups__valuation"><span><Value value={group.precio} money /> / unidad</span>{group.unidades_por_paquete !== null && group.precio_paquete !== null && <small><Valuation group={group} /></small>}<button type="button" className="icon-button" aria-label={`Editar valorizado de ${group.nombre}`} onClick={() => setValuationGroup(group.id)}><Pencil size={16} /></button></td><td><button type="button" className="icon-button" aria-label={`Editar máscara y categoría de ${group.nombre}`} onClick={() => setEdit(group.id)}><Pencil size={16} /></button></td></tr>)}{!visible.length && <tr><td colSpan={5}>No hay grupos para los filtros seleccionados.</td></tr>}</tbody></table></div>
