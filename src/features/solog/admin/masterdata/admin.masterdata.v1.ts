...
function group(value: unknown): value is MasterDataGroup { return object(value) && typeof value.id === 'string' && typeof value.nombre === 'string' && typeof value.categoria_id === 'string' && positive(value.precio) && ((value.unidades_por_paquete === null && value.precio_paquete === null) || (integer(value.unidades_por_paquete) && Number(value.unidades_por_paquete) > 1 && positive(value.precio_paquete))) }
function product(value: unknown): value is MasterDataProduct { return object(value) && integer(value.c_interno) && Number(value.c_interno) > 0 && typeof value.producto === 'string' && nullableString(value.c_barras ?? null) && nullableString(value.marca ?? null) && positive(value.precio) && ['Único', 'Agrupado', 'Excluido'].includes(String(value.estado)) && typeof value.categoria_id === 'string' && (value.grupo_id === undefined || value.grupo_id === null || typeof value.grupo_id === 'string') && (value.estado === 'Excluido' || typeof value.grupo_id === 'string') && (value.propuesta === undefined || object(value.propuesta) && ['excluir_producto', 'reincorporar_producto', 'eliminar_producto'].includes(String(value.propuesta.tipo)) && ['pendiente', 'aprobado'].includes(String(value.propuesta.estado)) && typeof value.propuesta.fingerprint === 'string') }
function setup(value: unknown): value is MasterDataSetupRequired { return object(value) && typeof value.cambio_id === 'string' && typeof value.propuesta_fingerprint === 'string' && ['agregar_producto', 'reincorporar_producto'].includes(String(value.tipo)) && integer(value.c_interno) && typeof value.producto === 'string' && positive(value.precio) && typeof value.block_reason === 'string' }
