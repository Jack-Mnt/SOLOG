...
describe('AdminDialog Fase 5 — formularios y tareas', () => {
  ...
  test('Descargar ajuste usa sede contextual y switch binario de quincena', async () => {
    ...
    expect(dialog).toContain("allowed_sites.find(candidate => candidate.id === siteId)")
    expect(dialog).toContain("adminSiteLabel(site.nombre)")
    expect(dialog).toContain('className="admin-dialog-context"')
    expect(dialog).toContain('<dt>Sede</dt>')
    expect(dialog).toContain('<dd>{siteName}</dd>')
