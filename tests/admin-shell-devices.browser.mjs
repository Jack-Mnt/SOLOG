...
await context.route('**/*', async route => {
  ...
  assert.ok(['rpc_solog_admin_bootstrap_v2', 'rpc_solog_operational_v2', 'rpc_solog_admin_devices_v2', 'rpc_solog_admin_incidents_v2'].includes(rpc), rpc)
...
try {
  ...
  assert.equal(await page.locator('.admin-workspace').getAttribute('data-admin-viewport'), 'tablet')
  ...
  assert.equal(await page.getByRole('button', { name: 'Alternar navegación' }).count(), 1)
  assert.equal(await page.locator('.admin-workspace').getAttribute('data-admin-viewport'), 'desktop')
  }
  await nav('Control')
  ...
  assert.equal(await page.getByRole('article', { name: 'Solicitud de Huaca', exact: true }).getByRole('button', { name: 'Reemplazar tablet', exact: true }).count(), 0)
  assert.equal(await page.getByRole('article', { name: 'Solicitud de Huaca', exact: true }).getByRole('button', { name: 'Autorizar', exact: true }).count(), 1)
  ...
  assert.equal(await appearance.getByRole('button').count(), 3)
  const mobileSidebarControlHeights = await mobileDrawer.locator('.admin-tab, .admin-appearance > button, .admin-sidebar__account .icon-button').evaluateAll(nodes => nodes.map(node => Math.round(node.getBoundingClientRect().height)))
  assert.ok(mobileSidebarControlHeights.length > 0 && mobileSidebarControlHeights.every(height => height === 40))
  await appearance.getByRole('button', { name: 'Natura', exact: true }).click()
  assert.equal(await page.evaluate(() => document.documentElement.dataset.palette), 'green')
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true)
  await screenshot('drawer-mobile-767')
  ...
  await drawerTrigger.click()
  await mobileDrawer.waitFor({ state: 'visible' })
  await screenshot('drawer-mobile-430')
  await mobileDrawer.getByRole('button', { name: 'Control', exact: true }).click()
  await page.getByRole('heading', { name: 'Control', level: 1 }).waitFor()
  await mobileDrawer.waitFor({ state: 'hidden' })
  assert.equal(await drawerTrigger.evaluate(node => document.activeElement === node), true)

  await drawerTrigger.click()
  await mobileDrawer.waitFor({ state: 'visible' })
  await mobileDrawer.getByRole('button', { name: 'Dispositivos', exact: true }).click()
  await page.getByRole('heading', { name: 'Dispositivos', level: 1 }).waitFor()
  await mobileDrawer.waitFor({ state: 'hidden' })
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true)
  ...
  await page.waitForFunction(() => document.querySelector('.admin-sidebar').getBoundingClientRect().width >= 190)
  await singleColumn()
  await screenshot('desktop-restored-expanded')
  ...
  assert.equal(await page.getByRole('article', { name: 'Solicitud de Huaca', exact: true }).getByRole('button', { name: 'Autorizar', exact: true }).isDisabled(), true)
  ...
  await nav('Dashboard'); await page.getByRole('heading', { name: 'Cutervo', exact: true }).waitFor()
  await nav('Control'); await page.locator('.admin-control').waitFor()
