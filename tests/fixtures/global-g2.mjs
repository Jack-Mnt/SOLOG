...
export function scenario() {
  const daily = responseFixture('daily_detail_bootstrap', { site_id: 'site-a', origin_date: '2026-09-15', stock_class: 'positive' })
  daily.generated_at = generated
  daily.views.Confirmada = [{ case_id: 'case-0', grupo: 'Grupo 0', difference: -2, valued_difference: -7.5 }]
  daily.counts.positive.Confirmada = 1

  const control = responseFixture('control_groups', { site_id: 'site-a', period: 'previous_biweekly' })
  control.generated_at = generated
  control.period = period
  control.items = [{ case_id: 'case-0', group_id: 'group-0', group_name: 'Grupo 0', category: 'Bebidas', origin_at: origin, state: 'Confirmada', difference: -2, valued_difference: -7.5 }]

  const admin = adminExport('site-a', 'previous_biweekly')
  admin.generated_at = generated
  admin.period = period
  admin.summary = { total: 1, coincide: 0, pending_recount: 0, confirmed: 1, inconsistent: 0 }
  admin.adjustments = [{ ...admin.adjustments[0], fecha_origen: origin }]
  admin.all = [{ ...admin.adjustments[0], recontado_at: recounted }]
  admin.pending_recount = []
  admin.inconsistent = []

  const details = detailsExport('previous_biweekly')
  details.generated_at = generated
  details.site = admin.site
  ...
  summary.site = admin.site
  summary.generated_at = generated
