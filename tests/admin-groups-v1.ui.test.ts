...
describe('Grupos V1 UI sobre Master Data', () => {
  test('usa una sola superficie derivada con tabla y filtros locales', async () => {
    ...
    expect(ui).toMatch(/<th scope=["']col["']>Grupo<\/th>\s*<th scope=["']col["']>Categoría<\/th>\s*<th scope=["']col["']>Integrantes<\/th>\s*<th scope=["']col["']>Valorizado<\/th>/)
  ...
  test('crea y opera integrantes con mutaciones V1 sin clasificar ni editar precio unitario', async () => {
    ...
    expect(ui).toMatch(/store\.mutation\(\s*["']group_create["']/)
    expect(ui).toMatch(/store\.mutation\(\s*["']group_update["']/)
    expect(members).toMatch(/store\.mutation\(\s*["']membership_move["']/)
    expect(members).toMatch(/store\.mutation\(\s*["']make_unique["']/)
