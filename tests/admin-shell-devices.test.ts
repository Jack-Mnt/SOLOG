...
test('Shell site selection is UI-only, retained across module loads, and scoped to this store', async () => {
  ...
  await store.load('control_groups', { site_id: store.siteId, period: 'today' })
  expect(store.siteId).toBe('site-b')
  expect(store.peek('control_groups', { site_id: 'site-b', period: 'today' }).data?.site_id).toBe('site-b')
