...
describe('AdminDialog Fase 8 — Drawers', () => {
  test('AdminDialog controla el ancho de drawer sin crear variantes nuevas', async () => {
    ...
    expect(dialog).toContain("export type AdminDialogKind = 'confirmation' | 'task' | 'management'")
  ...
  test('Cronología usa timeline y carga la quincena anterior solo al activar el switch', async () => {
    ...
    expect(css).toContain('.admin-control-chronology__timeline::before')
    expect(css).toContain('.admin-control-chronology__event > article::before')
    expect(css).toContain('.admin-control-chronology__event > article[data-tone="success"]::before')
    expect(css).toContain('.admin-control-chronology__event > article[data-tone="warning"]::before')
    expect(css).toContain('.admin-control-chronology__event > article[data-tone="danger"]::before')
    expect(css).toContain('.admin-control-chronology__event > article[data-tone="info"]::before')
  ...
  test('Repeticiones consume detalle agregado por sede sin paginación ni tabla', async () => {
    ...
    expect(incidents).not.toContain('useManagementQuery(\n    "detail"')
    expect(incidents).not.toContain('legacyFallback')
