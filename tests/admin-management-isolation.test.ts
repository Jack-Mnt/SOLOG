...
import { ManagementError, type managementRead, type managementMutate, type Payload } from '../src/features/solog/admin/admin.management.v2'
...
test('normalized transport errors preserve the original operation, not just raw network errors', async () => {
  ...
  await expect(store.mutation('revoke',{device_id:'site-a-device-0'},2,'site-a')).rejects.toThrow()
  expect(store.intent('devices')).toBeDefined(); await store.retryMutation('devices'); expect(sent[0]).toEqual(sent[1])
})
test('incident revision conflict clears the rejected intent and refreshes its source',async()=>{
  ...
  await store.load('summary',{});await expect(store.mutation('reactivate',{family_key:'fp',scope:'global'},4)).rejects.toThrow('REVISION');expect(store.peek('summary',{}).data).toBeUndefined()
...
test('authorization rejection clears data and notifies the Admin shell',async()=>{
  ...
  await store.load('summary',{});await expect(store.load('list',{})).rejects.toThrow('ROLE');expect(forbidden).toBe(true);expect(store.peek('summary',{}).data).toBeUndefined();await expect(store.load('summary',{})).rejects.toThrow('Contexto')
})
test('late response cannot cross a role change',async()=>{
  ...
  const pending=store.load('summary',{});auth.identity.rol='moderador';release(managementFixture('summary'));await expect(pending).rejects.toThrow('invalidada');expect(store.peek('summary',{}).data).toBeUndefined()
})
test('incident period expiration is anchored to backend time and leaves devices cached',async()=>{
  ...
  const store=new ManagementStore('admin-test',bootstrapFixture,()=>{},read,undefined,()=>clock)
...
test('different Auth users never share a management dataset',async()=>{
  ...
  const first=new ManagementStore('admin-test',()=>auth,()=>{},read);await first.load('summary',{})
  const other={...auth,identity:{...auth.identity,id:'other'}}
  const second=new ManagementStore('other',()=>other,()=>{},read);expect(second.peek('summary',{}).data).toBeUndefined()
  first.dispose();expect(first.peek('summary',{}).data).toBeUndefined()
})
