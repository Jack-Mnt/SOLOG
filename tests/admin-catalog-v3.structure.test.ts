...
describe('Catálogo V3: estructura principal', () => {
  ...
  test('Productos opera sobre Master Data completo y conserva las mutaciones Catálogo V3', async () => {
    ...
    expect(ui).toMatch(/action\s*===\s*["']exclude["'][\s\S]*?["']Aprobar exclusión["'][\s\S]*?:[\s\S]*?["']Aprobar reincorporación["']/)
    expect(ui).toContain('Esta acción aprueba el cambio, pero el producto no cambiará hasta publicar el Catálogo.')
    expect(ui).not.toContain('<dt>Modalidad</dt>')
    expect(ui).not.toMatch(/useCatalogQuery\(\s*["']products["']/)
...
test('la tabla de propuestas compone identidad y Cambio sin lecturas adicionales', async () => {
  ...
  expect(ui).toContain('admin-table-cell-stack')
  expect(ui).toContain('admin-table-cell-primary')
