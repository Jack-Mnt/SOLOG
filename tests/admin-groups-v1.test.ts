...
function setup(options: { mutate?: (action: string, payload: Record<string, unknown>) => unknown } = {}) {
  ...
  const read = (async (action, payload) => { calls.push({ channel: 'read', action, payload: structuredClone(payload) }); return fixture(action, payload) }) as GroupsReadTransport
  ...
  return { store: new GroupsStore('admin-test', () => auth, () => {}, read, mutate), calls }
}
...
describe('Grupos V1 contrato y store', () => {
  ...
  test('conflicto de revisión recarga autoridad y una nueva intención usa otro operation_id', async () => {
    let attempts = 0
    const { store, calls } = setup({ mutate: () => { if (!attempts++) { revisions = { groups: 4, catalog: 8 }; throw Object.assign(new Error('Revisión'), { code: 'SOLOG_MASTERDATA_REVISION_CONFLICT' }) }; return { ...envelope(), replay: false, result: {} } as GroupsMutationResult } })
    await store.load('status', {})
    await expect(store.mutation('make_unique', { c_interno: 100 })).rejects.toThrow('Revisión')
    expect(store.intent()).toBeUndefined()
    expect(calls.filter(call => call.channel === 'read').map(call => call.action)).toContain('reference')
  ...
  test('staging conflict no se reintenta y libera la intención para una nueva operación', async () => {
    const { store } = setup({ mutate: () => { throw Object.assign(new Error('Staging'), { code: 'SOLOG_CATALOG_STAGING_CONFLICT' }) } })
    await store.load('status', {})
