...
describe('Admin table normalization — Phase 3 Catálogo + Productos', () => {
  test('Catálogo usa main table en propuestas y conserva su tabla auxiliar interna fuera del alcance', async () => {
    ...
    expect(catalog).toContain('<th scope="col">Origen</th>')
    expect(catalog).toContain('className="admin-table-action-cell">Acción</th>')
  })
  ...
  test('Productos usa main table, precio numérico y contrato común de acciones', async () => {
    ...
    expect(products).toContain('className="admin-table-action-cell">Acción</th>')
  ...
  test('la composición funcional de ambas tablas permanece congelada', async () => {
    ...
    expect(catalog).toMatch(/Tipo[\s\S]*Producto[\s\S]*Cambio[\s\S]*Origen[\s\S]*Acción/)
    expect(products).toMatch(/Producto[\s\S]*C\. interno[\s\S]*Categoría[\s\S]*Grupo[\s\S]*Precio[\s\S]*Acción/)
  })
})
