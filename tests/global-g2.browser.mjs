...
try {
  ...
  await periodSwitch.waitFor({ state: 'attached' })
  const periodControl = page.locator('.export-period__control')
  assert.equal(await periodControl.getByText('Anterior', { exact: true }).isVisible(), true)
  assert.equal(await periodControl.getByText('Actual', { exact: true }).isVisible(), true)
  assert.equal(await periodSwitch.isEnabled(), true)
  assert.equal(await periodSwitch.isChecked(), true, 'Actual es el período inicial')
  assert.equal(await periodControl.locator('.is-active').textContent(), 'Actual')
  const beforePeriodChange = calls.length
  await periodSwitch.focus()
  for (const current of [false, true, false]) {
    await periodSwitch.press('Space')
    assert.equal(await periodSwitch.isChecked(), current)
    assert.equal(await periodControl.locator('.is-active').textContent(), current ? 'Actual' : 'Anterior')
  }
  assert.equal(calls.length, beforePeriodChange, 'Cambiar período es local, sin solicitudes')
  assert.equal(await page.getByRole('button', { name: 'Descargar Excel', exact: true }).isEnabled(), true)
  ...
  const detailsExports = calls.filter(call => call.rpc === 'rpc_solog_details_v2' && call.action === 'export')
  assert.equal(detailsExports.length, 1)
  assert.deepEqual(detailsExports[0].p, { period: 'previous_biweekly' })
