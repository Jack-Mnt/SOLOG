...
describe('Composición F3 de Grupos e Incidencias', () => {
  ...
  test('Incidencias parte de Pendientes y deriva las StateView desde summary', async () => {
    ...
    expect(ui).toContain('useState<IncidentState>("pendiente")')
    expect(ui).toContain('{ state: "pendiente", label: "Pendientes" }')
    expect(ui).toContain('{ state: "suprimida", label: "Suprimidas" }')
    expect(ui).toContain('{ state: "resuelta", label: "Resueltas" }')
