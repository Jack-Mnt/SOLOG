create or replace function inventario.solog_admin_catalog_mutate_v4(
  p_uid uuid,
  p_action text,
  p_payload jsonb
)
...
begin
  ...
  if p_action='proposal_action' then
    ...
    if found then
      if v_change.estado<>'pendiente' or v_change.tipo<>'precio' then
        raise exception 'SOLOG_CATALOG_CHANGE_NOT_PENDING';
      end if;
    else
      select *
    ...
    values(
      'resolve_and_approve_catalog_price_v4',
      'cambio_catalogo',
      v_change_id::text,
      'usuario',
      p_uid::text,
    ...
    return jsonb_build_object(
      'ok',true,
      'codigo','CATALOG_PRICE_RESOLVED_AND_APPROVED',
      'cambio_id',v_change_id,
      'propuesta_fingerprint',v_fp,
      'estado','aprobado',
      'tipo','precio',
  ...
  elsif p_action in ('prepare_product','prepare_price') then
    return inventario.solog_admin_catalog_mutate_v3(p_uid,p_action,p_payload-'operation_id');
...
revoke execute on function inventario.solog_admin_catalog_mutate_v4(uuid,text,jsonb)
...
begin
  ...
  if p_action='summary' then
    ...
    if p_action='ignore_30d' then
      ...
      v_fp:=inventario.solog_catalog_admin_proposal_fingerprint_v4(v_operation,v_inc.c_interno,'eliminar_producto');
...
revoke execute on function public.rpc_solog_admin_incidents_v2(text,jsonb) from anon;
grant execute on function public.rpc_solog_admin_incidents_v2(text,jsonb) to authenticated;
