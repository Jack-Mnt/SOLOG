create or replace function inventario.solog_control_chronology_view_v1(
  p_uid uuid,
  p_site uuid,
...
begin
  ...
  left join lateral (
    ...
    where c.sede_id=p_site and cd.grupo_conteo_id=v_group
  ...
  if v_group_name is null then raise exception 'SOLOG_INVALID_GROUP'; end if;
  ...
  functional_rows as (
    ...
    select
      ...
      null::numeric,
      null::numeric,
      case when c.estado_diferencia='Confirmada' then c.recount_difference::numeric else null::numeric end,
      case when c.estado_diferencia='Confirmada'
        then inventario.solog_calcular_valor_diferencia(
          c.recount_difference,c.precio,c.unidades_por_paquete,c.precio_paquete
        )
        else null::numeric
      end,
      case when c.estado_diferencia='Inconsistente' then c.stock_teorico_reconteo::numeric else null::numeric end,
      case when c.estado_diferencia='Inconsistente' then c.initial_difference::numeric else null::numeric end,
      case when c.estado_diferencia='Inconsistente' then c.recount_difference::numeric else null::numeric end,
      c.precio::numeric,
      2
    from cases c
    where c.estado_diferencia in ('Confirmada','Inconsistente')
      ...
      select jsonb_agg(
        case fr.state
          ...
          when 'Confirmada' then jsonb_build_object(
            ...
            'state',fr.state,
            'difference',fr.difference,
            'valued_difference',fr.valued_difference
          )
          when 'Inconsistente' then jsonb_build_object(
            'row_id',fr.row_id,
            'event_at',fr.event_at,
            'state',fr.state,
            'theoretical',fr.theoretical,
            'initial_difference',fr.initial_difference,
            'found_difference',fr.found_difference
          )
        end
        order by fr.event_at desc,fr.case_id desc,fr.stage_order desc
      )
...
revoke all on function inventario.solog_control_chronology_view_v1(uuid,uuid,timestamptz,jsonb) from public, anon, authenticated;
...
begin
  ...
  if p_action='control_groups' then
    ...
    return inventario.solog_control_chronology_view_v1(v_uid,v_site,v_generated,p_payload);
