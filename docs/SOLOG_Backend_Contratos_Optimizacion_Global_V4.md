# SOLOG — Contratos Backend de Optimización Global V4
...
> La versión **V4 de este documento** no cambia `contract_version = 2` de las APIs. V4 identifica la revisión documental/contractual.
...
3. **`SOLOG_Backend_Contratos_Optimizacion_Global_V4.md` — contrato de integración frontend vigente.**
4. `SOLOG_Backend_Contratos_Optimizacion_Global_V3.md` — **REEMPLAZADO / histórico**.
5. `SOLOG_Backend_Contratos_Optimizacion_Global_V2.md` — **REEMPLAZADO / histórico**.
6. `SOLOG_Backend_Contratos_Optimizacion_Global_V1.md` — **REEMPLAZADO / histórico**.
7. Motor y modelo de datos vigentes del proyecto donde este contrato no los sustituya explícitamente.

Ante contradicción entre versiones de contratos, **prevalece V4**.

### Alcance acumulado V2 + V3 + V4
...
- backend desplegado;
- C1–C3 pueden reanudarse sin consultas v1, KPI inventados ni recálculo frontend.

Codex puede reanudar el bloque coordinado C1 → C2 → C3 con estas restricciones:

- no modificar Supabase/backend;
- V4 reemplaza V3/V2/V1 como contrato primario;
- usar `panel_state` como fuente de grupos/colas/KPI del bootstrap Cajero;
- después de cada mutación usar el `state` autoritativo devuelto por backend;
- no incrementar/decrementar cobertura o KPI mediante reglas propias;
- `save_batch.state.kpis` ya refleja la cobertura actualizada;
- un replay no debe duplicar cambios locales;
- no crear consumidores nuevos v1;
- no acceder a `inventario` directamente;
- si aparece otra necesidad backend no cubierta por V4, detener la fase y devolverla a ChatGPT;
- preservar cambios preexistentes;
- no ejecutar refactors generales no relacionados.

C4 permanece fuera de este reinicio hasta completar y revisar C1–C3.
