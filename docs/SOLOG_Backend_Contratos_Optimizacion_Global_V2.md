# SOLOG — Contratos Backend de Optimización Global V2
...
**Migración de cierre V2:** `20260903112822_solog_backend_contracts_v2_route_master_reads`

> La versión **V2 de este documento** no cambia `contract_version = 2` de las APIs. V2 identifica la revisión documental/contractual.
...
3. **`SOLOG_Backend_Contratos_Optimizacion_Global_V2.md` — contrato de integración frontend vigente.**
4. `SOLOG_Backend_Contratos_Optimizacion_Global_V1.md` — **REEMPLAZADO / histórico**.
5. Motor y modelo de datos vigentes del proyecto donde este contrato no los sustituya explícitamente.

Ante contradicción entre V1 y V2 de contratos, **prevalece V2**.

### Alcance del cambio V2

V2 cierra los dos bloqueos descubiertos durante el preflight del plan frontend:

- **B1 — Login/routing:** se despliega un resolver universal v2 mínimo.
- **B2 — lecturas Admin:** se congela el payload real de Dashboard/turnos y se despliega una superficie v2 de lectura para Catálogo/Grupos/Publicación.
...
`device` y `start_capability` son estados autoritativos del backend. `session_state` contiene la sesión v2 congelada cuando existe.

`start_capability.allowed=false` puede acompañarse, entre otros, de:
...
Esta migración no creó ni modificó datos operativos. Las pruebas de lectura se realizaron sin mutaciones de negocio.

---

# 18. Gate E2E Cajero

El cierre B1/B2 no modifica el gate anterior.
...
- **B2 cerrado**;
- backend desplegado;
- contratos frontend necesarios completos para comenzar la implementación;
- no queda un bloqueo contractual conocido para I1, I2, C1–C4, D1–D3 o A1–A6.

Codex puede corregir el plan y, tras aprobación del usuario, implementar frontend con estas restricciones:

- no modificar Supabase/backend;
- V2 reemplaza V1 como contrato primario;
- no crear consumidores nuevos v1;
- no acceder a `inventario` directamente;
- no inventar campos/payloads;
- si aparece una necesidad backend no cubierta por V2, detener la fase y devolverla a ChatGPT;
