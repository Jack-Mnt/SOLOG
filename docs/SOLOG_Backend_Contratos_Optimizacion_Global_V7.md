# SOLOG — Contratos Backend de Optimización Global V7
...
- V6: `20260904051718_solog_catalog_publication_stable_artifact_v6`
- V7: `20260904123039_solog_cashier_review_batch_v7`

> La versión **V7 de este documento** no cambia `contract_version = 2` de las APIs. V7 identifica la revisión documental/contractual.
...
3. **`SOLOG_Backend_Contratos_Optimizacion_Global_V7.md` — contrato de integración frontend vigente.**
4. `SOLOG_Backend_Contratos_Optimizacion_Global_V6.md` — **REEMPLAZADO / histórico**.
5. `SOLOG_Backend_Contratos_Optimizacion_Global_V5.md` — **REEMPLAZADO / histórico**.
6. `SOLOG_Backend_Contratos_Optimizacion_Global_V4.md` — **REEMPLAZADO / histórico**.
7. `SOLOG_Backend_Contratos_Optimizacion_Global_V3.md` — **REEMPLAZADO / histórico**.
8. `SOLOG_Backend_Contratos_Optimizacion_Global_V2.md` — **REEMPLAZADO / histórico**.
9. `SOLOG_Backend_Contratos_Optimizacion_Global_V1.md` — **REEMPLAZADO / histórico**.
10. Motor y modelo de datos vigentes del proyecto donde este contrato no los sustituya explícitamente.

Ante contradicción entre versiones de contratos, **prevalece V7**.

### Alcance acumulado V2 + V3 + V4 + V5 + V6 + V7
...
V6 cierra el bloqueo descubierto durante el preflight de **A4 — reintento de publicación preparada**:

- el artefacto de una publicación `prepared` debe ser determinista para el mismo `operation_id`;
- la fecha embebida en el catálogo no puede depender de la hora de cada intento;
- la Edge usa el timestamp persistido de preparación (`prepared_at`) como fecha estable del artefacto;
- si el upload ya ocurrió y la ejecución se interrumpió antes del commit, el retry con el mismo `operation_id` reconstruye exactamente los mismos bytes y hash;
- un objeto existente con el mismo hash se acepta como upload ya realizado y el flujo continúa hacia commit.

V6 no cambia el request frontend de publicación ni `contract_version=2`.

V7 cierra los hallazgos backend descubiertos durante el smoke humano de **Cajero / Revisar**:

- `review_queue` expone la última diferencia y el `contado_at` del caso pendiente;
- la prioridad operativa de Revisar es la antigüedad del conteo original;
- los reconteos nuevos se capturan localmente y se persisten en un único lote mediante `recount_save_batch`;
- abrir o navegar por un caso de Revisar no requiere una escritura backend;
- el snapshot y stock teórico del reconteo se fijan al enviar el lote usando la sesión actual congelada;
- el lote es atómico e idempotente por `operation_id`;
- Motor V3 y la valorización final continúan siendo autoritativos en backend.

V7 es aditivo. `recount_start` y `recount_save` permanecen desplegados solo por compatibilidad temporal y son candidatos a S10 cuando no existan consumidores.
...
- cron existentes;
- Edge `conexion-admin` v4;
- mutaciones v2 previamente congeladas, salvo la nueva acción Cajero `recount_save_batch` definida en V7.
...
Array autoritativo de casos pendientes de reconteo:
...
- `ultima_diferencia` es la diferencia operativa vigente del `conteo_detalle` pendiente;
- `contado_at` es la fecha/hora del conteo original;
- solo contiene casos con `estado_diferencia='Recontar'`, primer snapshot posterior existente y sin `stock_reconteo`;
- orden obligatorio:
...
Los casos más antiguos aparecen primero.

Durante una sesión activa, la **membresía** de la cola continúa congelada mediante `solog_session_groups`. El frontend no debe reordenar por categoría, grupo, magnitud de diferencia ni valorización.
...
- `start`;
- `save_batch`;
- `recount_save_batch` — **V7 / acción vigente para el frontend nuevo**;
- `recount_start` — compatibilidad temporal;
- `recount_save` — compatibilidad temporal;
- `finish`.
...
### `recount_save_batch` — NUEVO V7

Esta es la acción que debe usar el frontend nuevo de **Revisar**.
...
- mínimo 1 y máximo 500 items;
- `detalle_id` no puede repetirse dentro del mismo lote;
- `stock_fisico >= 0`;
- `contado_at` debe pertenecer a la sesión vigente y no estar en el futuro más allá de la tolerancia existente;
- cada detalle debe pertenecer a un caso realmente pendiente `Recontar`;
- cada grupo debe formar parte del freeze de la sesión actual con `requiere_reconteo=true`;
- la sesión de origen del caso debe ser anterior a la sesión actual;
- un caso originado en la misma sesión devuelve `SOLOG_RECOUNT_SAME_SESSION_FORBIDDEN`.

#### Freeze del reconteo V7

Al **enviar el lote**, no al abrir el modal:

```text
snapshot_reconteo_id = snapshot_referencia_id de la sesión actual
stock_teorico_reconteo = stock_teorico congelado del grupo en esa sesión
```

Si existe metadata previa de `recount_start` pero todavía no existe `stock_reconteo`, V7 la sustituye por el freeze de la sesión que efectivamente envía el reconteo.

Esto permite que:

- abrir Revisar sea solo lectura/local;
- navegar Anterior/Siguiente no escriba;
- la UI no tenga que esperar una RPC por cada caso;
- el lote use una única referencia coherente.
...
- `Dr = 0` → `Coincide`, diferencia final `0`;
- D0 y Dr con el mismo signo → `Confirmada`, diferencia final = la de menor magnitud;
- signos opuestos → `Inconsistente`, diferencia final = `Dr`.

El trigger de valorización recalcula `valor_diferencia` usando la diferencia final y los valores congelados:

- `precio`;
- `unidades_por_paquete`;
- `precio_paquete`.

El frontend puede mostrar una previsualización local, pero el resultado persistido es autoritativo del backend.

#### Atomicidad e idempotencia

`recount_save_batch` es una única transacción:

- si un item falla, **ningún item del lote queda persistido**;
- el backend incrementa `operational` una sola vez al completar el lote;
- el mismo `operation_id` con el mismo payload devuelve la respuesta comprometida con `replay:true`;
- un retry no ejecuta nuevamente los reconteos ni duplica efectos.
  ...
  "items": [
    {
      ...
      "stock_teorico_reconteo": 10,
      "stock_reconteo": 9,
      "diferencia_reconteo": -1,
      "diferencia": -1,
...
`state` es el estado autoritativo completo de la sesión después de aplicar todos los items.

### `recount_start` — LEGACY TEMPORAL

Se conserva para consumidores históricos, pero **el frontend Cajero nuevo no debe llamarlo**.

Su contrato anterior permanece compatible hasta S10.

### `recount_save` — LEGACY TEMPORAL

Se conserva para consumidores históricos, pero **el frontend Cajero nuevo no debe llamarlo**.

Motor V3 y sus validaciones anteriores permanecen compatibles hasta S10.
...
- no `upsert` cliente;
- `rpc_solog_catalog_publication_v2` sigue siendo interna;
- el frontend no conoce ni envía `prepared_at`, hash, ruta o timestamp del artefacto.

### Artefacto estable V6

Al recibir estado `prepared`, la Edge consulta internamente:

`rpc_solog_catalog_publication_artifact_v6(actor_id, operation_id)`

La RPC devuelve metadata de la reserva:
...
`prepared_at` corresponde a `inventario.solog_catalog_publications.created_at` y es inmutable para esa operación.

La Edge usa ese timestamp para construir:

- `fecha` dentro del JSON del catálogo, convertida a `America/Lima`;
- `generado_at` enviado al commit;
- indirectamente, el contenido comprimido y su hash.

Por tanto, para la misma reserva:
...
+ productos
+ prepared_at
= mismo artefacto byte-a-byte
= mismo hash
= misma ruta
```

### Recuperación después de upload y antes de commit

Escenario contractual:

1. `begin` devuelve `prepared`;
2. la Edge construye el artefacto con `prepared_at`;
3. Storage acepta el upload;
4. la ejecución se interrumpe antes del commit;
5. el cliente reintenta con el mismo `operation_id`;
6. `begin` devuelve nuevamente la misma reserva `prepared`;
7. la Edge obtiene el mismo `prepared_at`;
8. reconstruye exactamente el mismo artefacto y hash;
9. el segundo upload con `upsert:false` encuentra el objeto existente;
10. la Edge descarga el objeto, verifica que `existingHash === hash`;
11. continúa hacia `commit_catalog_publication`.

Ese caso **no** debe producir `CATALOG_UPLOAD_CONFLICT`.

`CATALOG_UPLOAD_CONFLICT` queda reservado para un objeto existente en la ruta esperada cuyo contenido realmente no coincide con el artefacto reservado.

### Seguridad interna V6

`rpc_solog_catalog_publication_artifact_v6(uuid,uuid)`:
...
No forma parte de la superficie frontend.
...
## A4 — Recuperación de publicación `prepared`

**RESUELTO EN V6.**

El defecto era real: la Edge anterior usaba la hora del intento para `fecha`, por lo que un retry tras upload podía reconstruir un hash distinto y convertir una recuperación válida en `CATALOG_UPLOAD_CONFLICT`.

V6 fija el artefacto a `prepared_at`, persistido desde la reserva.


## Cajero — Revisar por lote y cola cronológica

**RESUELTO EN V7.**

El smoke humano mostró dos carencias contractuales reales:

1. `review_queue` no exponía `ultima_diferencia` ni `contado_at`;
2. `recount_start/recount_save` obligaban al frontend a escribir caso por caso.

V7:

- enriquece la cola;
- la ordena por antigüedad del conteo original;
- añade `recount_save_batch`;
- mueve el freeze efectivo del reconteo al envío del lote;
- mantiene las acciones unitarias solo como compatibilidad temporal.

---

# 17. Evidencia de validación V7
...
### Publicación preparada V6

Preflight confirmado:

- la Edge v3 generaba `fecha` desde `new Date()` en cada intento;
- dos reintentos con distinta hora producían contenido y hash distintos;
- una reserva `prepared` conserva `created_at`, por lo que existe una fuente temporal estable.

Despliegue:

- migración `20260904051718_solog_catalog_publication_stable_artifact_v6`;
- Edge `conexion-admin` actualizada a versión `4`;
- `verify_jwt=true` preservado.

Validación de determinismo con la misma reserva sintética:

```text
same_fecha = true
same_hash = true
same_bytes = true
existingHash === retryHash → resume_commit
```

La prueba reconstruyó el artefacto dos veces en instantes distintos usando el mismo `prepared_at`. Los bytes y SHA-256 fueron idénticos.

Estado productivo previo al despliegue:

- publicaciones con `status='prepared'`: `0`.

Por ello no existía una publicación interrumpida que requiriera migración/reparación de artefacto.
...
No se ejecutó una publicación real durante la validación para no incorporar cambios de catálogo ni escribir un artefacto productivo artificial.

### Cajero Revisar V7

Despliegue:

- migración `20260904123039_solog_cashier_review_batch_v7`;
- `contract_version` permanece en `2`;
- no se añadieron tablas ni columnas.

Validación dinámica de `review_queue` en Huaca:

- 17 casos pendientes en el momento de la comprobación;
- todos los elementos incluyeron `grupo_id`, `detalle_id`, `ultima_diferencia` y `contado_at`;
- orden `contado_at ASC` validado;
- los primeros casos correspondieron efectivamente a los conteos más antiguos.
...
Validación transaccional descartable de `recount_save_batch` usando casos reales pendientes y una sesión/dispositivo sintéticos revertidos al finalizar:

```text
success_batch = true
saved = 2
resultado 1 = Coincide, diferencia = 0, valorizado = 0
resultado 2 = Confirmada, diferencia = 1, valorizado = 5
review_pending después del lote = 1
replay_same_operation = true
atomicity_on_second_item_failure = true
forced_rollback_left_no_synthetic_writes = true
```

Para atomicidad se ejecutó un lote donde el primer item era válido y el segundo inexistente. El backend devolvió `SOLOG_RECOUNT_NOT_PENDING` y la mutación del primer item fue revertida junto con todo el lote.

La misma intención repetida con el mismo `operation_id` devolvió `replay:true`.

La validación descartable comprobó además que al finalizar:

- no quedó sesión sintética;
- no quedó ledger sintético;
- el hash del dispositivo volvió a su valor original;
- ningún caso productivo quedó recontado por la prueba.
...
V7 actualiza el gate humano para el flujo nuevo de Revisar.

Secuencia operativa objetivo:

```text
start
→ capturas locales de Conteo
→ save_batch
→ comprobar que los casos nacidos en esa sesión NO aparecen todavía en Revisar
→ finish
→ snapshot posterior confirmado
→ nueva sesión
→ capturas locales de Revisar
→ recount_save_batch
→ finish
```
...
- diferencia y valorización preview en frontend antes del envío;
- `review_queue` más antiguo primero;
- navegación por toda la cola sin llamadas unitarias de escritura;
- envío único por bloque;
- resultado autoritativo posterior al batch;
- estados finales coherentes;
- ausencia de reconteo en la misma sesión de origen.

En el momento del smoke debe existir:

- snapshot confirmado y vigente;
- dispositivo real autorizado;
- `device_token` real;
- al menos un caso elegible `Recontar` para validar el batch.

Las validaciones sintéticas V7 no sustituyen este smoke humano.
...
- V2–V6 continúan acumulativamente vigentes donde V7 no los reemplaza;
- `review_queue` queda enriquecida y ordenada cronológicamente;
- `recount_save_batch` está desplegado y validado;
- `recount_start/recount_save` quedan como legacy temporal;
- backend del delta Cajero queda listo para implementación frontend;
- el smoke humano final debe ejecutarse después de migrar la UI.

Codex puede implementar el delta Cajero aprobado con estas restricciones:

- no modificar Supabase/backend;
- **V7 reemplaza V6/V5/V4/V3/V2/V1 como contrato primario**;
- usar `review_queue.ultima_diferencia` y `review_queue.contado_at`;
- respetar el orden recibido del backend;
- no llamar `recount_start`;
- no llamar `recount_save`;
- guardar reconteos como drafts locales;
- enviar mediante `recount_save_batch`;
- mantener el mismo `operation_id` solo al reintentar exactamente el mismo lote;
- reemplazar estado local con `state` autoritativo tras éxito/replay;
- no recalcular Motor V3 en frontend;
- la valorización frontend es únicamente preview con datos congelados;
- no introducir consumidores v1;
- preservar cambios preexistentes;
- no realizar refactors generales no relacionados;
- si aparece otra incompatibilidad backend, detenerse y devolverla a ChatGPT.

G3 permanece pendiente hasta completar el delta y el smoke humano.  
S10 permanece diferido.

No ejecutar limpieza legacy todavía.
...
| Publicación catálogo | Edge `conexion-admin` v4 |

`rpc_solog_catalog_publication_v2` permanece interna al servicio de publicación y no forma parte de la superficie frontend.

En Cajero, la superficie pública continúa siendo `rpc_solog_cashier_mutate_v2`; V7 añade la acción `recount_save_batch`. Las acciones unitarias `recount_start` y `recount_save` siguen desplegadas únicamente por compatibilidad temporal.
