...
Revoca anticipadamente la supresión correspondiente al scope solicitado.

- `global`: revoca la exclusión global de la familia;
- `site`: revoca la exclusión de esa sede;
...
El `status='active'` de la respuesta significa **supresión levantada para el scope**, no garantiza que la anomalía siga actualmente presente.
...
Los campos de resolución/actividad son aditivos.
...
El backend queda preparado para que el frontend Admin > Incidencias se adapte contra este contrato sin requerir cambios adicionales conocidos.
