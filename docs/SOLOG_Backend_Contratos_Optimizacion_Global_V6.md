# SOLOG — Contratos Backend de Optimización Global V6
...
> La versión **V6 de este documento** no cambia `contract_version = 2` de las APIs. V6 identifica la revisión documental/contractual.
...
3. **`SOLOG_Backend_Contratos_Optimizacion_Global_V6.md` — contrato de integración frontend vigente.**
4. `SOLOG_Backend_Contratos_Optimizacion_Global_V5.md` — **REEMPLAZADO / histórico**.
5. `SOLOG_Backend_Contratos_Optimizacion_Global_V4.md` — **REEMPLAZADO / histórico**.
6. `SOLOG_Backend_Contratos_Optimizacion_Global_V3.md` — **REEMPLAZADO / histórico**.
7. `SOLOG_Backend_Contratos_Optimizacion_Global_V2.md` — **REEMPLAZADO / histórico**.
8. `SOLOG_Backend_Contratos_Optimizacion_Global_V1.md` — **REEMPLAZADO / histórico**.
9. Motor y modelo de datos vigentes del proyecto donde este contrato no los sustituya explícitamente.

Ante contradicción entre versiones de contratos, **prevalece V6**.

### Alcance acumulado V2 + V3 + V4 + V5 + V6
...
- B1/B2 cerrados;
- bloqueos Cajero cerrados;
- bloqueo A2 `shift_grid` cerrado;
- **bloqueo A4 de recuperación de publicación preparada cerrado**;
- backend y Edge desplegados;
- A4–A6 pueden reanudarse.

Codex puede reanudar A4 → A5 → A6 con estas restricciones:

- no modificar Supabase/backend;
- V6 reemplaza V5/V4/V3/V2/V1 como contrato primario;
- publicación frontend conserva `{action:'publish_catalog', operation_id}`;
- mantener el mismo `operation_id` al reintentar la misma publicación lógica;
- no generar ni enviar timestamps/hash/rutas desde frontend;
- no cambiar UUID para escapar de un estado `prepared`;
- no subir archivos directamente desde frontend;
- consumir replay/errores autoritativos de la Edge;
- no crear consumidores v1;
- si aparece otra incompatibilidad backend real, detenerse y devolverla a ChatGPT;
- preservar cambios preexistentes;
- no realizar refactors generales no relacionados.

No avanzar a Integración Global al terminar A6 sin revisión.
